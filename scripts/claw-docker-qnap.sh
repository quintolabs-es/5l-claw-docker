...
MANAGED_DOWNLOAD_SPECS=(
  ...
  ".openclaw/skills/authenticate-github-repo/SKILL.md:.openclaw/skills/authenticate-github-repo/SKILL.md"
  ".openclaw/skills/authenticate-github-repo/scripts/authenticate-github-repo.sh:.openclaw/skills/authenticate-github-repo/scripts/authenticate-github-repo.sh"
...
rewrite_openclaw_version_in_file() {
  ...
  rewrite_file_with_sed "$path" "s|^ARG[[:space:]]*OPENCLAW_VERSION=.*$|ARG OPENCLAW_VERSION=${escaped_version}|"
}
...
require_host_commands bash curl sed awk mktemp head basename dirname chmod mv rm mkdir cat env
