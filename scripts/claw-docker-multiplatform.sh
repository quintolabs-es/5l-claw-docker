...
# Usage:
#   bash ./scripts/claw-docker-multiplatform.sh init
#   bash ./scripts/claw-docker-multiplatform.sh init --port 19001
#   bash ./scripts/claw-docker-multiplatform.sh update
#   bash ./scripts/claw-docker-multiplatform.sh update --port 19001
...
#   curl -fsSL "https://raw.githubusercontent.com/quintolabs-es/5l-claw-docker/main/scripts/claw-docker-multiplatform.sh?skip-cache=$(date +%s)" | bash -s -- update --port 19001

ROOT_DIR="${PWD}"
RAW_BASE_URL="${CLAW_DOCKER_RAW_BASE_URL:-https://raw.githubusercontent.com/quintolabs-es/5l-claw-docker/main}"
SELF_PATH_RELATIVE="scripts/claw-docker-multiplatform.sh"
...
MANAGED_DOWNLOAD_SPECS=(
  ...
  "scripts/claw-docker-multiplatform.sh:scripts/claw-docker-multiplatform.sh"
)
...
EXECUTABLE_MANAGED_FILES=(
  ...
  "scripts/claw-docker-multiplatform.sh"
)
...
usage() {
  cat <<'EOF'
Usage:
  scripts/claw-docker-multiplatform.sh init [--port <port>]
  scripts/claw-docker-multiplatform.sh update [--port <port>]
EOF
}
...
assert_directory_empty() {
  ...
  if (( ${#entries[@]} > 0 )); then
    echo "Error: target directory is not empty: ${root_dir}. Use scripts/claw-docker-multiplatform.sh update for existing projects." >&2
    exit 1
  fi
}
...
refresh_self_for_update() {
  ...
  if ! curl -fsSL "${RAW_BASE_URL}/${SELF_PATH_RELATIVE}" -o "${TMP_SELF_SCRIPT}"; then
    echo "Error: failed to download the latest scripts/claw-docker-multiplatform.sh for update." >&2
    exit 1
  fi
